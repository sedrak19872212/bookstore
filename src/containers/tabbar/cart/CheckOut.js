...
export default function CheckOut({navigation, route}) {
    ...
    const onPressPayment = () =>{
        console.log("ppppppppppppppPayment1");
    let date = moment().format('YYYY-MM-DD H:mm:ss');
    let uniqueId = Math.floor(Date.now() / 1000); // Unix timestamp

    set(ref(db, 'orders/' + user.uid + '/' + uniqueId), cartData);

       dispatch(deleteAllItemsAction(cartData));

    console.log("ssssssssssssssSet");
